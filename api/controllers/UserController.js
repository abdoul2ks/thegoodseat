...
module.exports = {
  register:function(req,res) {
    ...
    if (Users.password !== Users.Cpassword) {return res.status(400).json({'password': 'you have entered différent password !'});}
  ...
  Login:function(req,res) {
    ...
    if (!Users.username || !Users.password) {
      return res.status(400).json('the fields cannot be empty');
    } else {
      User.find({where: {username:Users.username},})
        .then(users => {
          if (!users) {
            return res.status(400).json('loginMessage: No User Found please create one !');
          } else {
